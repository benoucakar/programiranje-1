...
let dollar_to_euro_dolgo dollar = match dollar with
...
(* Ne more se zgodit da dollarja ne bo pretvorili v evro ali pa da bi ga pretvoril večkrat *)
...
let beno = {name= "Beno"; status= Student (Fire, 4.0)}
...
let update ({arcane=vrednost_arcane} as magic_counter) magic_type = 
...
(* Če ima spremenljivka isto ime kot polje:

let update ({arcane} as magic_counter) magic_type = 
    match magic_type with
    | Fire -> {fire=magic_counter.fire + 1; frost=magic_counter.frost; arcane=magic_counter.arcane}
    | Frost -> {magic_counter with frost=magic_counter.frost + 1}
...
let count_magic l =
    let rec count_pomozna trenutno delovni_seznam = 
        match delovni_seznam with
            | [] -> trenutno
            | x::rest -> 
...
let rec find_candidate magic specialization = function
    | [] -> None
    | wizard::rest -> (
        match wizard.status with
            | Student (student_magic, years) when student_magic=magic ->(
                if zahtevana_leta specialization <= years then Some wizard.name
