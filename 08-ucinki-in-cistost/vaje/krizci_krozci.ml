...
(* Tu je kul, ker nimamo mutable tipov, in se nebodo vsi empty rowi hkrati spreminjal *)
...
let is_full_row_drugace (x1, x2, x3) = not (x1 = None || x2 = None || x3 = None)

(*
= preveri če je enako npr a = 3, b=3, -> a = b
== preveri, če je isto, npr x == x
*)
...
let is_full_grid grid =
    ...
    (* funkcija all da "and" med vse elemente seznama *)
...
(* Triples je (field * field * field) list *)
(* Poje triple poljubniga tipa, vrne player option*)
...
let place_token player grid (row_i, col_i) : state =
  ...
  let updated_grid = set_field row_i col_i (Some player) grid in (* Tu še ne preverimo, če smo povozili rešeno polje *)
...
let show_state = function
  ...
  | GameOver {result; final_grid} ->
      let winner_message = (match result with Tie -> "Ni zmgovalca" | Winner p -> show_player p) in
